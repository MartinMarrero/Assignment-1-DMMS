import { Pessimistic, Optimistic, Laplace } from './decisionmethod';
export class DecisionController {
    ...
    handleCompute(csv, method, opts) {
        ...
        try {
            ...
            console.log('Controller textarea reference in view (if available):', window.__lastViewTextarea || null);
        }
        catch (e) {
            console.log('Error while inspecting DOM textareas', e);
        }
        // Debug: expose parsed table (names) as a first row so the UI shows what was parsed
        // This helps diagnose empty parsing in some locales/inputs.
        const parsedNames = table.map(t => t.name).join(',');
        const results = [];
        results.push({ method: 'parsed', name: parsedNames, value: table.length });
        ...
        const hur = this.executeHurwitz(table, degree);
        results.push({ method: 'hurwitz', name: hur[0], value: hur[1] });
        ...
        try {
            ...
            window.__hurwitzTable = { headers, rows };
        }
        catch (e) {
            window.__hurwitzTable = null;
        }
        this.onResult(results);
    }
    executeHurwitz(table, degree) {
        if (table.length === 0)
            return ['', 0];
        let best = table[0];
    ...
    parseCSV(text) {
        ...
        for (let i = 0; i < lines.length; ++i) {
            ...
            if (/example|csv format|header row|then two rows/.test(low))
                continue;
        ...
        let nameStart = 0;
        if (!header[0] || /outcome|alternativ|^\\/.test(header[0].toLowerCase()))
            nameStart = 1;
        ...
        for (let li = headerIndex + 1; li < lines.length && rowIndex < 2; ++li) {
            ...
            if (cells.length === table.length) {
                for (let i = 0; i < cells.length && i < table.length; ++i) {
                    ...
                    if (rowIndex === 0)
                        table[i].outcomes.first = val;
                    else
                        table[i].outcomes.second = val;
                }
            }
